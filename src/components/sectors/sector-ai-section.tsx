...
function SectorMemberList({ sector }: { sector: SectorTheme }) {
  return (
    <div className="bg-emerald-50 dark:bg-emerald-950/20 rounded-[20px] p-5 border border-emerald-100 dark:border-emerald-900/30">
      <h4 className="font-bold text-sm text-emerald-800 dark:text-emerald-400 mb-3 flex items-center gap-1.5">
        <TrendingUp className="w-3.5 h-3.5" />섹터 주요 종목
      </h4>
      <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        {sector.memberSymbols.map(sym => (
          <li key={sym}>
            <Link
              href={`/stocks/${sym}`}
              className="flex items-center justify-between gap-3 rounded-xl bg-white px-3 py-2 shadow-sm ring-1 ring-emerald-100 transition-colors hover:ring-emerald-200 dark:bg-zinc-900 dark:ring-emerald-900/40 dark:hover:ring-emerald-800"
            >
              <span className="text-sm font-semibold text-slate-800 dark:text-zinc-100">{getStockName(sym)}</span>
              <span className="text-[10px] text-emerald-500/70 font-mono">{sym}</span>
            </Link>
          </li>
        ))}
      </ul>
...
function SectorAIContent({ snapshot, sector }: { snapshot: SectorResearchSnapshot; sector?: SectorTheme }) {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {sector && <SectorMemberList sector={sector} />}
