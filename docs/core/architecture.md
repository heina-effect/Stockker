...
### KIS 응답 봉수 한도와 지수 페이지네이션 (Phase 37)

KIS 기간별시세 엔드포인트는 단일 호출당 반환 봉수에 상한이 있다. 측정값 기준 **업종 일자별 지수(`FHKUP03500100`)는 약 50봉, 주식 일봉/주봉(`FHKST03010100`)은 약 100봉**이다. 오버나이트 거시필터는 KOSDAQ 120일 정배열(5>20>60>120)을 판정하므로 지수 단일 호출(50봉)로는 MA120을 계산할 수 없어, 거시필터가 `kosdaqClose=0`으로 비활성화되던 버그가 있었다.

- `getDomesticIndexDaily()`는 **날짜 창을 과거로 옮겨가며 페이지네이션**해 120봉 이상(목표 130봉, 최대 4페이지)을 누적한다. `stck_bsop_date`로 중복 제거 후 최신순으로 정렬한다.
- 결과는 **하루 단위 캐시**다. 캐시 키에 KST 날짜를 포함(`index_daily_{code}_{YYYYMMDD}`)하고 TTL 26h를 둬, 거래일당 1회만 신규 조회한다.
...
- **개별 종목의 120일 정배열 기준은 변경하지 않는다.** 페이지네이션은 거시필터 지수에만 적용한다.
