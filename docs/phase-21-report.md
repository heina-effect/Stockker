...
## 6. 추가 개선 사항 (Hotfixes)
### A. 내 평단가 기준 대응 가이드 수익률 계산 오류 수정
- **문제**: 백엔드 `generateBuyPlan` API가 실시간 시세가 아닌 하드코딩된 mock 가격(75,000원)을 사용하여 예상 수익률을 잘못 계산하던 문제를 해결했습니다.
- **해결**: `getDomesticStockQuote`를 연동하여 실제 UI에 표시되는 시세와 동일한 소스를 기반으로 수익률을 계산하도록 변경했습니다.
- **분석 로직 세분화**: 수익률 구간별(손실, 약손실, 약수익, 수익)로 AI 진단 문구와 액션 가이드를 더욱 정교하게 매칭되도록 업데이트했습니다.
