...
## 4. 추가 개선 사항 (Hotfixes & UX Improvements)

### A. 분석 출처 데이터 중복 제거 및 시계열 정확도 개선
- **중복 제거**: 수집된 뉴스들 중 제목이 동일한 경우(공백 제거 기준) 사용자에게 중복 노출되지 않도록 `vector-store.ts` retrieval 단계에서 필터링 로직을 보완했습니다.
- **타임존 보정**: KIS 뉴스 및 DART 공시의 KST 시간을 UTC(`Z`)로 잘못 처리하던 로직을 수정(`+09:00`)하여 실제 발행 시간을 정확하게 표시하도록 개선했습니다.
- **UI 게시일 우선**: 리서치 카드에서 수집일(`collectedAt`) 대신 실제 기사 발행/공시일(`generatedAt`)을 시:분 단위까지 상세하게 노출하도록 변경했습니다.

### B. 검색 결과 시장(Market) 식별 고도화
- **KOSPI/KOSDAQ 판별**: 마스터 데이터에 없는 종목도 종목 코드 패턴(00xxxx -> KOSPI, 그 외 -> KOSDAQ)을 분석하여 자동으로 라벨을 부여하는 Heuristic을 도입했습니다.
- **뱃지 시각화 차별화**: 검색 결과에서 종목(KOSPI/KOSDAQ)과 섹터(SECTOR)를 명확히 구분할 수 있도록, 섹터 검색 결과에는 **퍼플 컬러 뱃지**를 적용하여 시각적 인지도를 높였습니다.

## 5. 결론
이번 페이즈를 통해 Stockker는 단일 KIS 뉴스 의존도를 벗어나, GNews와 NewsAPI를 아우르는 멀티 뉴스 파이프라인을 갖추게 되었습니다. 또한 수집된 Raw 뉴스가 즉시 사용되지 않고 **저장 → 임베딩 큐레이션 → 검증된 데이터 재사용**이라는 구조화된 워크플로우를 통과함으로써, 앱 전체의 신뢰도와 설명 가능성(Explainability)이 대폭 향상되었습니다.
